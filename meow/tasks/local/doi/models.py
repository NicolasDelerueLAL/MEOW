...
class ContributionDOI:
    ...
    def _build_hep_attributes(self) -> dict:
        attributes = {
            "titles": [{"source": "JACOW", "title": self.title}],
            "abstracts": [{"source": "JACOW", "value": self.abstract}],
            "imprints": [{"date": format_datetime_dashed(datetime_now(self.timezone))}],
            "keywords": [{"source": "JACOW", "value": k} for k in self.keywords],
            "publication_info": [
                {
                    "artid": self.code,
            ...
            "dois": [{"source": "JACOW", "value": self.doi_name}],
            ...
            "authors": [
                {
                    ...
                    "raw_affiliations": [
                        {"value": str(a)} for a in author.affiliations
                    ],
                }
    ...
    def _build_doi_attributes(self) -> dict:
        ...
        attributes["descriptions"] = [
            dict(description=self.abstract, descriptionType="Abstract", lang="en-us")
        ]
